...
const info=JSON.parse(fs.readFileSync(APP+"/kody_rozbicie_p67.json","utf8"));
const plan=JSON.parse(fs.readFileSync(APP+"/plan_p67.json","utf8"));
...
const bak=`${DB}.bak_full_pre_scalanie_p67_${ts}`;
...
db.close();
fs.writeFileSync(APP+`/audit_scalanie_p67_${ts}.json`, JSON.stringify(audit,null,1));
console.log("Zaktualizowano:",touched,"| KI:",updKi,"| nazwa:",updNa,"| audit: audit_scalanie_p67_"+ts+".json");
console.log("GOTOWE");
