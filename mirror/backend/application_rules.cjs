...
function normalizeApplication(category, value) {
  ...
  const mapped = splitApplications(value)
    .map((item) => APPLICATION_ALIASES[normalizeKey(item)] || null)
    .filter(Boolean);

  const specific = [...new Set(mapped.filter((item) => item !== UNIVERSAL && allowed.has(item)))];
  if (specific.length > 0) {
    return CATEGORY_VALUES[canonical].filter((item) => specific.includes(item)).join(' ; ');
  }
  return UNIVERSAL;
}
