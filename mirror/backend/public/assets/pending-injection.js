...
(function() {
  ...
  async function tick() {
    ...
    if (currentRoute !== state.lastRoute) {
      ...
      ensureButtons();
      // Jeżeli nie ma naszego widoku, wyrenderuj
  ...
  async function openProductsModal(rodzaj, wartosc) {
    ...
    try {
      ...
      products.forEach(p => {
        ...
        html += `<td style="text-align:right;"><a class="catalog-link" href="#/katalog?q=${encodeURIComponent(p.kod || '')}">Katalog &rarr;</a></td>`;
        html += '</tr>';
      });
      html += '</tbody></table>';
      body.innerHTML = html;
  ...
  function init() {
    ...
    window.addEventListener('hashchange', scheduleAttributesFix);
