...
w układzie „co zmieniliśmy → polecenie → rezultat" (polecenie Ani z 22.09). Jedenaście punktów
do sprawdzenia, trzy wyraźnie oznaczone sprostowania wcześniejszych obietnic i sześć rozbieżności
w sekcji „Do Twojej decyzji". Przy okazji wykonany triaż zmian produkcji.

## Problem / Motivation
Ania wdrożyła we wrześniu na produkcji szereg zmian, a my odtworzyliśmy je w `rebuild/`. Potrzebny
był dokument, który mówi **gdzie je zobaczyć** — nie test systemu (ten powstaje równolegle jako
TEST.1/TEST.2), tylko delta. Karta `docs/karty/I15.9/` (karta + 11 wejść od kart I15).

## Solution
- **Nowy:** `docs/instrukcja-testow-I15.md` — rozdz. 1 (8 zmian Ani), rozdz. 2 (2 nasze zmiany),
  rozdz. 3 (tabela sprostowań), rozdz. 4 (6 decyzji dla Ani), rozdz. 5 (podsumowanie), rozdz. 6.
- `docs/triage-state.txt` — marker przesunięty na `5bd4a7b`.
- `docs/karty/I15.9/karta.md` — karta zamknięta, obalony `7d6cfc9` usunięty, 3 zgłoszenia
  do koordynatora.
- **Nowe:** `docs/karty/TEST.1/wejscie-151.md`, `docs/karty/TEST.2/wejscie-151.md`.

## Design decisions
- **Forma wg polecenia Ani z 22.09** — krótko, bez ściany tekstu; z wzorców I7-v2/I10-v2 zostaje
  tylko rusztowanie do raportowania („Twoja ocena" + tabela zbiorcza).
- **Trzy sprostowania oznaczone dwukrotnie** — przy punkcie i w tabeli rozdz. 3: szerokość,
  EAN naukowy, pełne pliki CSV z Analityki.
- **Nie powielamy ścieżki krytycznej** — przy MO9, Staging v2 i CSV zostaje „jest teraz tak"
  plus odesłanie do TEST.2.
- **Warunki środowiskowe wg stanu z 24.09** (decyzja użytkownika), nie wg `wejscie-113.md`;
  rozjazd odnotowany, nie ukryty.
- **Trzy dodatkowe punkty włączone** decyzją użytkownika: pełne pliki CSV z Analityki, kolejka
  atrybutów −526, import zatrzymujący się na błędach odczytu.

## Triaż zmian produkcji (wykonany w tym tickecie)
`88fa31c..origin/main` = **5 commitów `sync(vps)`** (23.09, 14:00–18:00), każdy dotyka wyłącznie
`sellycsv-*.csv`. **Zero kodu** → nowych wpisów backlogu nie ma. Nagłówek CSV bajt w bajt
identyczny na obu końcach (sprawdzone celowo — kolumna blokowanych form płatności jest w zakresie
I15). **Zakres I15 pozostaje domknięty na `88fa31c`.**

## Tests
- **Gate odbudowy (fixtures/kontrakt): N/D** — ticket nie dotyka API.
- **Bramki backendu: N/D, weryfikowalnie** — `git diff --name-only origin/develop...HEAD`
  filtrowane po `^docs/` zwraca pustkę; `rebuild/` jest identyczne z `origin/develop`. Nie były
  uruchamiane i raport tego nie udaje.
- **Weryfikacja treści:** każda nazwa ekranu, zakładki, filtra, kolumny, przycisku i każdy
  komunikat sprawdzone w kodzie (tabela źródeł w `raport.md`).
- **Recenzja: dwa obiegi.** Obieg 1: 1 BLOCKER + 2 SHOULD-FIX — wszystkie naprawione.
  Obieg 2: 0 BLOCKER / 0 SHOULD-FIX.

## Breaking changes
None.

## Follow-up
1. **`docs/karty/I15.9/wejscie-122.md` zawiera nieprawdziwy przykład** treści kolumny blokowanych
   form płatności — to źródło BLOCKER-a z recenzji. Cudzy plik, zgłoszone koordynatorowi.
2. **Odwołanie „I3 §11 pkt 10" nie istnieje** (`rebuild-backlog.md:3879`, `wejscie-120.md`) —
   prawdziwe miejsce to `instrukcja-testow-I3.md` §4 pkt 3.
3. **`wejscie-113.md` nieaktualne** w punkcie o wyłączonym schedulerze importu.
4. **Trzy nieaktualne zdania z `wejscie-144.md` pkt 3** — nadal nierozliczone.
5. **Sześć decyzji Ani** z rozdziału 4 wraca do nas po jej odpowiedzi.
6. ⚠ **Trzy odsyłacze wskazują pliki, których jeszcze nie ma w `develop`**
   (`instrukcja-testu-sciezki-krytycznej.md`, `instrukcja-pelnego-testu.md`,
   `instrukcja-pracy-dla-ani.md` — tickety 149/150). Muszą wejść, zanim kartka pójdzie do Ani.
...
---
Ticket docs: `docs/tickets/151-DOCS-instrukcja-testow-i15/`
