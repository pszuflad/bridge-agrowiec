...
pasek narzędzi złożony w jeden rząd zgodnie z `fe.js:20702-20770` (akcje masowe przeniesione
...
- **Nowy:** `rebuild/frontend/src/pages/staging/kolumny.ts` — lista 61 kolumn 1:1 z `STAGING_COLS`
  (`fe.js:28808-29105`), `KOLEJNOSC_KOLUMN` (`POS_KEYS`), klucz `bridge_staging_cols_v2`
...
- Doszło `modal={false}` na `DropdownMenu`. **To nie jest ustępstwo na rzecz testów, tylko
  wierność**: popover oryginału był zwykłym `<div>` w `body` (`fe.js:29176`), nie blokował
