...
| 3 | Import — rdzeń | 3a·3b·3c·3d BE · 3e FE | 2 | 🔨 | 3a: PR #6 · 2026-08-26 · 3b: PR #7 · 2026-08-26 |
...
- **Status:** 🔨 (3a ✅ 2026-08-26 · 3b ✅ 2026-08-26)  **Sesje (5, bottom-up):** 3a BE (port+charakteryzacja) · 3b BE (staging) · 3c BE (dopasowanie `tk()`) · 3d BE (zatwierdzanie+overrides) · 3e FE (`/staging`)  **Zależy od:** 2
...
- **3c · Silnik `tk()` — dopasowanie + klasyfikator** (BE, spec-backend §5, `03_IMPORT_tk.md`) — dopasowanie **kod → EAN → `Lq()`** (zastępczy, tylko opona); klasyfikator `Zc()` „czy opona"; normalizacja EAN `Hq()`; wykrywanie błędnego zapisu nazwy `Kq()`; porównywarka `Xq()` i etykiety pól `Vq`; klasyfikacja `nowa`/`blad`/`zmiana_kluczowa`. EAN auto tylko dla długości 8/12/13/14 i nie kończący się 5 zerami.
  - **⚠ Korekta zakresu (wniesiona przy planowaniu 3c, 2026-08-26):** pierwotny opis tej sesji wymieniał port `bridge_ext.cjs`/`tire_dims.js` oraz `kod_importu = bridge_ext.assignKodImportu`. Sprawdzenie grafu wywołań pokazało, że **żadne z nich nie jest wołane w zakresie 3c**: `assignKodImportu` wyłącznie w `addProductsBulk` (`:44746`) i `acceptStaging` (`:44827`), a `applyDims`/`applyLinkMemory` w ŻYWYM `tk()` wyłącznie w gałęzi auto-zatwierdzania (`:47794`) — wystąpienia `:47544-47545` leżą w martwym `function tk`. Ani `Zc()`, ani `Hq()` ich nie dotykają. **Przeniesione do 3d** (decyzja użytkownika, 2026-08-26). To DRUGI raz, kiedy `bridge_ext` był przypisany do złej sesji — pierwszy raz wykryła to 3a; stąd zasada w §3 o weryfikowaniu przypisania grafem wywołań.
  - **⚠ Pułapka: DWIE definicje `Lq()`** — `function Lq(t)` (`:46965`) liczy cyfry znaczące, a `function Lq(t, e)` (`:47312`) generuje identyfikator techniczny przez sha1 z `ean|nazwa|rozmiar|marka|model`. **Wygrywa `:47312`**, dokładnie jak `tk = function` z `:47584` wygrywa nad `function tk` z `:47378`.
  - **⚠ Wejście z 3b — CZYTAJ PRZED STARTEM.** 3b zostawiła gotowy szew; 3c wymienia CIAŁO jednej funkcji, nie architekturę.
    - **Punkt podmiany:** `silnikStagingu3b()` w `rebuild/backend/src/import/tk.ts`. Typ `SilnikStagingu = (kodDostawcy, surowe) => StatystykiImportu` i zestaw 10 liczników zostają bez zmian — są częścią kontraktu HTTP, bo endpointy importu rozsypują je wprost do ciała odpowiedzi (`...tkResult`). Trasy (`routes/import.ts`) i repozytorium (`repos/staging.ts`, `zapiszPozycjeStagingu` — zapis wsadowy w JEDNEJ transakcji) NIE wymagają zmian; silnik jest wstrzykiwany przez `ZaleznosciImportu.silnik`.
    - **Zakres większy, niż wymienia punkt wyżej:** dochodzą `Hq()` (normalizacja EAN → `eanRaw`/`eanIsValid`/`eanSourceStatus`/`eanCandidates` + `rozmiarWykryty`) i `Kq()` (wykrywanie błędnego zapisu nazwy, składnik `ostrzezenie`). W 3b cztery pola `ean*` są NULL-em.
    - **`snapshotJson` musi zmienić zawartość.** Oryginał serializuje rekord PO `Hq()` (zmienna `d`, `backend-index.cjs:47723`), a 3b serializuje surowy `RekordSurowy` sprzed normalizacji. To zmiana widoczna dla 3d, bo `acceptStaging` czyta z tego snapshotu (m.in. `uwagaCena`).
    - **NIE portować drugi raz:** filtr śmieci MO2 (`:47619-47634`) jest już w `tk.ts` jako port 1:1 — jedyny fragment `tk()` bez zależności zewnętrznych.
    - **Do zastąpienia, nie do zachowania:** 3b przy braku `kod` używa EAN-u jako identyfikatora, a rekord bez kodu i bez EAN-u odrzuca jako `odrzuconeBrakDanych`. To uproszczenie pełnego łańcucha `Zc()` → `Lq()` z oryginału (`:47643-47670`).
    - **Zależność od 3d — ROZSTRZYGNIĘTA (2026-08-26):** `Gq()` (overrides Marty) jest wołane WEWNĄTRZ `tk()`, przed `Zc()` (`:47676`), i zwraca `{pozycja, naruszono, srcVals}`. **3c stawia przepuszczający stub** (`pozycja` = wejście, `naruszono: []`, `srcVals: {}`) w jednym jawnie oznaczonym miejscu; realne overrides wymienia 3d. Konsekwencja do odnotowania w raporcie 3c: ostrzeżenie „plik nadpisuje poprawkę Marty" i gałąź `_srcConflict` nie powstaną do czasu 3d.
    - **Gate 3b świadomie NIE sprawdzał treści pozycji** (`typZmiany`, `powod`, pola `ean*`, zawartość `snapshotJson`) — testował warstwę odczytu na danych zasianych z fixtures. **To 3c ma dowieźć gate treści**: realny import pliku → porównanie zapisanych wierszy z oczekiwaniem. Próbki realnych cenników MO1–MO10 są w `rebuild/backend/test/charakteryzacja/probki/`.
    - **Bezpiecznik pustego wejścia przenosimy DO `tk()` (decyzja 2026-08-26).** Odstępstwo D4 z 3b (0 pozycji z parsera → błąd, bez zapisu) siedzi dziś w endpointach importu i nie zakrywa `POST /api/staging/import` (`:48502-48512`), który bierze pozycje wprost z ciała żądania. 3c przepisuje `tk()`, więc to jedna linijka w miejscu zakrywającym wszystkie trzy wejścia naraz — także trasę, która powstanie dopiero w 3d. Rozszerza to świadome odstępstwo D4 na cały silnik; odnotować w raporcie 3c.
  - **Gate:** testy scenariuszy dopasowania — nowy / po kodzie / po EAN / zastępczy / nie-opona.
- **3d · Zatwierdzanie + historia + wycofanie + overrides Marty** (BE) — **port `bridge_ext.cjs`/`tire_dims.js`** (przeniesiony tu z 3c przy planowaniu 3c, 2026-08-26 — patrz „Korekta zakresu" w bloku 3c: to jedyna sesja, która te moduły realnie woła); `applyDims`/`applyLinkMemory` w gałęzi auto-zatwierdzania (`:47794`); `kod_importu` = `bridge_ext.assignKodImportu` (grupa po EAN lub marka+rozmiar+bieznik+nazwa) w `addProductsBulk` (`:44746`) i `acceptStaging` (`:44827`); auto-zatwierdzenie **tylko** cena/marża/stan/magazyn → `historia_cen`; wycofanie po **3 kolejnych** nieobecnościach (`nieobecnosc_pod_rzad`, próg=3); **overrides `Gq()`** wpięte jako **strażnik**: przy konflikcie zachowuje wartość Marty (zapis do `snapshotJson`), override wygrywa. Staging: `POST /api/staging/accept`, `/reject`, `/import`, `/clear`; `GET /api/overrides`, `PUT/DELETE /api/overrides/{id}`.
  - **⚠ Wejście z 3b — CZYTAJ PRZED STARTEM.**
    - **Dziura w bezpieczniku #8 — domykana w 3c (decyzja 2026-08-26).** `POST /api/staging/import` (`backend-index.cjs:48502-48512`) bierze pozycje **wprost z ciała żądania** (`body.surowe ?? body.items ?? []`) i sprawdza wyłącznie `Array.isArray`, więc pusta tablica idzie prosto do `tk()` — żądanie `{dostawcaKod:"MO5"}` bez `surowe` uruchamia scenariusz z backlogu #8 (+1 do `nieobecnosc_pod_rzad` dla każdej pozycji dostawcy, po trzech wywołaniach wycofanie całego katalogu). Bezpiecznik przenosi do `tk()` sesja 3c, więc **ta trasa rodzi się już chroniona** — ale przy jej pisaniu potwierdź testem, że pusta tablica NIE dociera do stagingu ani do liczników.
    - **`products.uwaga_cena` czeka gotowa** (migracja `002_import.sql`, backlog #4). Do zrobienia: odczyt `uwagaCena` ze `snapshotJson` w `acceptStaging` + endpoint `GET /api/products/uwagi-cena`. **Uwaga:** endpointu NIE MA w zamrożonym kontrakcie — dopisanie do `openapi.yaml` należy do I12. Kolumna jest dziś ukryta przed API przez jawną projekcję (`src/repos/kolumny.ts`); jej ujawnienie w `GET /api/products` złamie gate I2, więc też dopiero po przenagraniu fixtures w I12.
    - **Backlog #3 (`szerokosc` REAL→TEXT) — decyzja należy do tej sesji**, bo `acceptStaging` jest jedynym pisarzem tej kolumny. Warunek wstępny: poprawka `parseWidthFallbackMm()` u Ani (dziś `szertxt` jest niekompletny) oraz świadomość, że zmiana typu łamie gate I2 do czasu przenagrania `GET_products.json` w I12.
    - **Dane stagingu z ery 3b są niepełne** — wiersze zapisane przed 3c mają `snapshotJson` bez pól `ean*` i `typZmiany` zawsze `"nowa"`. Przy testowaniu `acceptStaging` na starych danych deweloperskich trzeba je przeimportować, nie łatać.
    - **Wymiana stuba `Gq()`** — 3c stawia przepuszczający stub (decyzja zaklepana, patrz blok 3c); tutaj wchodzą realne overrides, a wraz z nimi ostrzeżenie „plik nadpisuje poprawkę Marty" i gałąź `_srcConflict` w `snapshotJson`.
...
- **3e · `/staging` (FE) + weryfikacja Ani** — widok `/staging`: przegląd pozycji importu, accept/reject, podgląd różnic. **Decyzja:** staging auto-accept vs ręczny (spec-frontend §4).
  - **⚠ Wejście z 3b — CZYTAJ PRZED STARTEM.**
    ...
    - **Dopóki nie ma 3c, staging zawiera wyłącznie pozycje `typZmiany: "nowa"`** — filtrów po `zmiana_kluczowa`/`wycofana`/`blad` i podglądu różnic nie da się przeklikać na realnych danych. Fixtures (`GET_staging_paged.json` ma `wycofana`) nadają się do MSW; pełny cykl z Anią wymaga 3c i 3d.
