...
- **3c · Silnik `tk()` — dopasowanie + klasyfikator** (BE, spec-backend §5, `03_IMPORT_tk.md`) — port `bridge_ext.cjs`/`tire_dims.js` (przeniesiony tu z 3a, decyzja D4 w `docs/tickets/4-FEATURE-port-parserow-charakteryzacja/`); dopasowanie **kod → EAN → `Lq()`** (zastępczy, tylko opona); klasyfikator `Zc()` „czy opona"; `kod_importu` = `bridge_ext.assignKodImportu` (grupa po EAN lub marka+rozmiar+bieznik+nazwa); `applyDims`/`applyLinkMemory` (`tire_dims.js`); EAN auto tylko dla długości 8/12/13/14 i nie kończący się 5 zerami.
  - **⚠ Wejście z 3b — CZYTAJ PRZED STARTEM.** 3b zostawiła gotowy szew; 3c wymienia CIAŁO jednej funkcji, nie architekturę.
    ...
    - **⚠ Zależność od 3d:** `Gq()` (overrides Marty) jest wołane WEWNĄTRZ `tk()`, przed `Zc()` (`:47676`), i zwraca `{pozycja, naruszono, srcVals}`. 3c nie zamknie wiernego `tk()` bez niego — trzeba świadomie wybrać: przepuszczający stub `Gq` (`naruszono: []`, `srcVals: {}`) do wymiany w 3d, albo wciągnięcie overrides wcześniej. **Decyzja do podjęcia na starcie 3c.**
...
- **3d · Zatwierdzanie + historia + wycofanie + overrides Marty** (BE) — auto-zatwierdzenie **tylko** cena/marża/stan/magazyn → `historia_cen`; wycofanie po **3 kolejnych** nieobecnościach (`nieobecnosc_pod_rzad`, próg=3); **overrides `Gq()`** wpięte jako **strażnik**: przy konflikcie zachowuje wartość Marty (zapis do `snapshotJson`), override wygrywa. Staging: `POST /api/staging/accept`, `/reject`, `/import`, `/clear`; `GET /api/overrides`, `PUT/DELETE /api/overrides/{id}`.
  - **⚠ Wejście z 3b — CZYTAJ PRZED STARTEM.**
    - **DZIURA W BEZPIECZNIKU #8 do domknięcia tutaj.** Bezpiecznik z 3b (pusty wynik parsowania → 400, bez zapisu) siedzi w `POST /api/import/parse-file` i `/from-url`. Ale `POST /api/staging/import` (`backend-index.cjs:48502-48512`) bierze pozycje **wprost z ciała żądania** (`body.surowe ?? body.items ?? []`) i sprawdza wyłącznie `Array.isArray` — **pusta tablica idzie prosto do `tk()`**. Żądanie `{dostawcaKod:"MO5"}` bez `surowe` uruchamia więc dokładnie ten scenariusz z backlogu #8, który 3b miała zablokować: +1 do `nieobecnosc_pod_rzad` dla każdej pozycji dostawcy, a po trzech takich wywołaniach wycofanie całego katalogu. Bezpiecznik trzeba powtórzyć na tej trasie albo — lepiej — przenieść go do samego `tk()`, gdzie zakryje wszystkie ścieżki naraz.
    ...
    - Jeśli 3c postawi przepuszczający stub `Gq()`, to **tutaj** jest jego wymiana na realne overrides.
