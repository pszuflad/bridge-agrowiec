...
| **Status** | zatwierdzone; **I15.2 (część parserowa) — ✅ DOWIEZIONA** (ticket 120, 2026-09-23): `staging_policy.cjs` w `legacy/` skopiowany bajt w bajt, adapter/common używają `validateEan`, `rawEan`, `syntheticCode`; `install()`/`registerRoutes()` leżą UŚPIONE (nikt ich nie woła). **I15.4a (schemat) — ✅ DOWIEZIONA** (ticket 124, 2026-09-23): tabela `staging_matches` i indeks unikalny `staging_one_current_product` założone migracją `012`. **I15.4b (silnik) — ✅ DOWIEZIONA** (ticket 130, 2026-09-23): nadpisania `assignKodImportu` (pierwsza gałąź DOSŁOWNIE — „zachowaj istniejący sześciocyfrowy `kod_importu`"), `addStaging` (nowe zgłoszenie ZASTĘPUJE poprzednie dla pary dostawca+kod), `updateStaging`, `updateProduct`. Otwarte: **I15.4c** (akceptacja i trasy) — `docs/karty/I15.4c/` |
...
| **Commit** | `abe5f14` |
| **Do nowej wersji?** | ✅ **TAK — decyzja użytkownika 2026-09-23 (D-130.1)** |
| **Status** | 🔨 **częściowo.** ✅ **CSV → ticket 122 (I15.3, 2026-09-23)** — zapis atomowy naniesiony (`generator-csv.ts`, tmp+`renameSync` w tym samym katalogu); filtr „tylko aktywne” odbudowa miała już od I8a, więc nie było tu czego zmieniać. ✅ **schemat → ticket 124 (I15.4a, 2026-09-23)** — tabela `product_auto_suspensions` założona migracją `012`. ✅ **logika auto-wstrzymań: I15.4b** (ticket 130, 2026-09-23) — brak w poprawnej pełnej ofercie ustawia `wstrzymany`+stan 0 natychmiast, `product_auto_suspensions` odróżnia automat od ręcznego, „pewny powrót” przywraca TYLKO automatycznie wstrzymane, ręczne są chronione (jawna zmiana `status` przez `PATCH /api/products/:id` kasuje znacznik automatu); `suspended_at` NIE jest aktualizowane przy konflikcie. Zostaje: `availability_sync` i delta → **I15.10** (punkt wpięcia wystawiony, domyślnie no-op). |
...
| **Status** | ✅ **parser: I15.2** — `adapter.cjs`/`mo9_agrorami_api.cjs` resyncowane bajt w bajt; W2 Handlopeksa zadziałało **15×** na pełnych cennikach (MO4 1, MO5 14 — próbki 200-wierszowe pokazywały tylko 2), `_supplierEanOriginal` niesie oryginał dostawcy. ✅ **staging: I15.4b** (ticket 130, 2026-09-23) — edycja modelu w stagingu aktualizuje bieżnik, jeśli był jego automatyczną kopią; ochrona partii DOT w dopasowaniu (`separateDotBatch`, różnica `dot` zrywa dopasowanie po kodzie). ⚠ Ustalenie: parser MO1 (Bohnenkamp) stempluje KAŻDY rekord wartością `dot = "nie starsza niz 3 lata"` (stała gwarancja z nagłówka cennika, nie numer partii) — karta katalogowa bez tej wartości nie dopasuje się więc do żadnego rekordu MO1; w produkcji nie występuje, bo karty zakłada ten sam importer. |
...
| **Commit** | `a2c979b` |
| **Do nowej wersji?** | ⬜ **do decyzji** |
| **Status** | 🔍 **zmierzone w I15.4b** (ticket 130, 2026-09-23): nowy silnik porównuje KAŻDĄ kartę katalogu z KAŻDYM rekordem cennika przez `compatibility()` w pętli nieobecnych (dla MO2 1729 kart × 200 rekordów, dla MO5 1989 kart), a `assignKodImportu` czyta całą tabelę `products` przy każdym wywołaniu (wiernie wobec `U.listProducts()` w `staging_policy.cjs:145`). Skutek mierzalny: przypadki cennikowe w charakteryzacji wymagały podniesienia limitu czasu z 20 s na 90 s. Nie rozstrzyga, czy problem #107 nas dotyczy — pomiar zatwierdzania zbiorczego zostaje zadaniem **I15.4c**. |
...
| **Status** | ⚠ żywy: 80 grup / 174 produkty · **przyczyna USTALONA (niżej)** · Ania (23.09) poprosiła o listę przypadków przed decyzją — lista wysłana, czeka na jej przegląd · **gałąź „zachowaj istniejący sześciocyfrowy `kod_importu`” przeniesiona DOSŁOWNIE w I15.4b (ticket 130, 2026-09-23)**, zgodnie z `docs/karty/I15.4b/wejscie-116.md` — wpis pozostaje OTWARTY, rozstrzygnięcie należy do Ani |
