...
function applyDims(P, rozmiarFallback) {
  try {
    ...
    // POPRAWKA 2026-08-19: NIE uzywamy juz tireWidthMm() jako fallbacku.
    // tire_dims.js's calcDims dla notacji "AxB" / "AxB-D" traktuje A jako srednice
    // felgi, B jako szerokosc — to konwencja ATV. Ale wiekszosc opon rolniczych
    // uzywa notacji "WxD" gdzie W to SZEROKOSC, D to srednica (np. 13.6x24 =
    // szerokosc 13.6", felga 24"). tireWidthMm dawal wtedy 24*25.4 = 609.6mm
    // dla wszystkich opon *xD niezaleznie od szerokosci. Poza tym zwracal w mm,
    // co miesza jednostki (Anna chce zostawic oryginalna). Zostawiamy null gdy
    // parseSize nic nie wykryl — wolimy null niz zle dane.
    return d;
  } catch (_) { return null; }
}
