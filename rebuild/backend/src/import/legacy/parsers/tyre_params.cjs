...
// wartosci) musi zwracac mm, tak jak size.szerokosc po naprawie nizej - inaczej surowa kolumna
// CSV (cale, np. 11.2) wraca nieprzeliczona i psuje jednostke w products.szerokosc.
function parseWidthFallbackMm(value) {
  const n = parseNumber(value);
  if (n === null) return null;
  const mm = n >= 100 ? n : n * 25.4;
...
// POPRAWKA 2026-08-18 (decyzja Anny): pola etykiety UE typu "snow" sa flaga Tak/brak,
// NIE liczba/ilosc - normalizeQty() tutaj bylo zle uzyte (obcinalo do 0/1, ktore trafialo
// jako string "0.0"/"1.0" do kolumny TEXT w bazie). Ta funkcja zwraca wylacznie 'Tak' albo
// null, niezaleznie czy surowa wartosc to liczba (0/1), string ("1"/"tak") czy bool.
function normalizeLabelFlag(value) {
  if (value === null || value === undefined || value === '') return null;
  const truthy = value === true || value === 1 || value === '1' ||
    (typeof value === 'string' && value.trim().toLowerCase() === 'tak');
...
function parseSize(value) {
  ...
  if (result.szerokosc !== null && result.srednica !== null) {
    ...
    // POPRAWKA 2026-08-19: cofniete przeliczanie szerokosc->mm. Anna zdecydowala,
    // ze `szerokosc` ma pozostac w JEDNOSTCE ORYGINALNEJ z rozmiaru (mm dla slash,
    // cale dla WxD/W-D). Panel pokazywal wtedy "706.2"/"609.6" dla opon 14.9x28
    // / 13.6x24, co bylo mylace: bug w tire_dims.js (calcDims dla AxB traktuje A jako
    // srednice, B jako szerokosc) sprzedawal przez tireWidthMm srednice zamiast szerokosci.
    // Teraz zostawiamy `result.szerokosc` = surowa pierwsza liczba z rozmiaru (14.9, 800, 13.6).
...
function parseTechnicalMarks(...values) {
  ...
  return {
    ...
    // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): HIGH SPEED -> skrot HS w nazwie (analogicznie do SB).
    hs: /\bHS\b|\bHIGH\s*SPEED\b/.test(upper) ? 1 : 0,
...
function normalizeJmk(record) {
  ...
  // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): FarmPro/FarmPRO/FARMPRO -> FARM PRO,
  // GRIPEX -> GRIP EX, i usuniecie pelnych fraz STEEL BELTED/HIGH SPEED z modelu (token SB/HS
  // dopisywany osobno w nazwaKoncowa nizej, nie zostaje w tekscie modelu).
  if (model) {
    model = model.replace(/\bfarm\s*pro\b/gi, 'FARM PRO');
    model = model.replace(/\bGRIPEX\b/gi, 'GRIP EX');
    model = model.replace(/\bsteel\s*belted\b/gi, '').replace(/\bhigh\s*speed\b/gi, '');
  ...
  const result = {
    ...
    szerokosc: size.szerokosc ?? parseWidthFallbackMm(record.szerokosc),
...
function normalizeGrasdorf(record) {
  ...
  // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): FarmPro/FarmPRO/FARMPRO -> FARM PRO,
  // GRIPEX -> GRIP EX, i usuniecie pelnych fraz STEEL BELTED/HIGH SPEED z modelu (token SB/HS
  // dopisywany osobno do nazwaKoncowa nizej, na podstawie marks.sb/marks.hs). Zastosowane PO
  // obu sciezkach (modelFromField ORAZ rawFallback z nazwy), zeby dzialalo niezaleznie od tego
  // ktore pole bylo wypelnione w CSV Grasdorfa.
  if (model) {
    model = model
      .replace(/\bfarm\s*pro\b/gi, 'FARM PRO')
      .replace(/\bGRIPEX\b/gi, 'GRIP EX')
      .replace(/\bsteel\s*belted\b/gi, '')
      .replace(/\bhigh\s*speed\b/gi, '')
      .replace(/\s+/g, ' ')
      .trim() || null;
  }
  // POPRAWKA 2026-07-21 (decyzja Anny): usuwamy z MODELU (pole model/bieznik) nawias
  // zawierajacy rozmiar/wymiar alternatywny, np. "(28x)", "(28x9-15)", "(17.5R24)", "(460/70)".
  // Zdarza sie gdy "Rodzaj bieznika" jest puste i fallback z nazwy wciaga alt-size do modelu
  // (np. "8.15-15 (28x9-15) BKT Maglift LIP" -> model "(28x) Maglift LIP"). Alt-size nalezy
  // do pola rozmiar_alternatywny, nie do modelu. NIE dotyczy nazwy koncowej (budowana osobno).
  if (model) {
    model = model
      .replace(/\(\s*[0-9][^)]*\)/g, ' ')
      .replace(/\s+/g, ' ')
  ...
  // POPRAWKA 2026-07-21 (decyzja Anny): standaryzacja Alliance "VP" -> "VALUE PLUS"
  // (VP to oficjalny skrot Alliance od Value Plus) oraz wielkosc liter "Value Plus" -> "VALUE PLUS",
  // zeby model/bieznik byl spojny z nazwa i standardem drukowanych liter (jak "320 VALUE PLUS").
  // "VP" tylko jako osobny token (\b), zeby nie ruszyc np. "VPxxx" w innych modelach.
  if (model) {
    model = model
      .replace(/\bvalue\s*plus\b/gi, 'VALUE PLUS')
      .replace(/\bVP\b/g, 'VALUE PLUS')
      .replace(/\s+/g, ' ')
      .trim() || null;
  }
  ...
  // POPRAWKA 2026-07-21: doklejamy record.rodzajBieznika do wykrywania SB/SF/HS - Grasdorf
  // czasem wpisuje "STEEL BELTED"/"HIGH SPEED" tylko w kolumnie "Rodzaj bieznika", nie w nazwie.
  const allMarks = parseTechnicalMarks(record.name, record.additionalName, record.iloscPlocien, record.rodzajBieznika);
  ...
  const result = {
    ...
    // (POPRAWKA 2026-07-21: dodano hs do result, uzywane w nazwaKoncowa nizej)
...
// Decyzja Anny 2026-07-21: '0' (oraz '0.0','00','-') w indeksie = BRAK indeksu -> null, nie zapisujemy 0
function nullIfZeroIndex(value) {
  if (value === null || value === undefined) return null;
  const t = String(value).trim();
  if (t === '' || t === '-') return null;
  // czyste zero (0, 0.0, 00, 0,0) traktujemy jako brak indeksu
...
function parseHandlopexLoadSpeed(loadRaw, speedRaw) {
  ...
  if (parsed.indeksNosnosci && parsed.indeksPredkosci) {
    return { indeksNosnosci: nullIfZeroIndex(parsed.indeksNosnosci), indeksPredkosci: nullIfZeroIndex(parsed.indeksPredkosci) };
  }
  if (loadText && speedText) {
    const load = loadText.replace(/[A-Z]\d?/gi, '');
    return {
      indeksNosnosci: nullIfZeroIndex(load || null),
      indeksPredkosci: nullIfZeroIndex(speedText)
    };
  }
  return { indeksNosnosci: nullIfZeroIndex(loadText || null), indeksPredkosci: nullIfZeroIndex(speedText || null) };
}
...
function extractHandlopexModel(nazwa, marka, size) {
  ...
  text = text
    ...
    //   V-5501 (Mitas)
    // POPRAWKA 2026-07-21 (Handlopex): chronimy litere/os po myslniku z obu stron -
    // oznaczenia typu WINMILE-D, WINLOAD-S, WINMILE-T (D=drive,S=steer,T=trailer) gubily
    // litere, bo negative lookahead sprawdzal tylko myslnik+CYFRA (np. R-4), nie myslnik+LITERA.
    // Dodano ochrone: nie usuwaj litery jesli PRZED nia jest myslnik ("(?<!-)") lub PO niej
    // jest myslnik+litera ("(?!-[A-Za-z])") — analogicznie do istniejacej ochrony myslnik+cyfra.
    .replace(/(?<![\w-])[A-Z]\d?\b(?!\s*-\s*\d+\b)(?!-[A-Za-z])(?<!-)/g, ' ')
  ...
  // POPRAWKA 2026-07-21 (Handlopex, Anna): usuwamy indeks nosnosci/predkosci, ktory wpada do modelu,
  // np. "165A6", "148A8/145B" (Handlopex czasem doklejaja go do nazwy modelu). NIE ruszamy oznaczen
  // typu "LS-2"/"TRS-2" (to czesc nazwy modelu) - flagi SF/SB/HF/LS/HS usuwa pozniej stripTechMarks.
  text = text
    .replace(/\b\d{2,3}[A-Z]\d?(?:\s*\/\s*\d{2,3}[A-Z]\d?)?\b/g, ' ')  // indeks nosnosci np. 165A6, 148A8/145B
    .replace(/\s+/g, ' ').trim();
...
function normalizeAgrorami(record) {
  ...
  // POPRAWKA 2026-07-21 (V-FLEXA/V-FLECTO, decyzja Anny): doklejamy record.nazwa —
  // Agrorami wpisuje VF/IF jako osobny token w pelnej nazwie handlowej (np. "Opona BKT
  // V-FLEXA TL 710/50R30,5 VF / (178D)"), NIE w bieznik (ktory ma tylko "V-FLEXA",
  // a \bVF\b nie dopasowuje "V-FLEXA" bo to jeden token z myslnikiem). Bez nazwy
  // flaga VF/IF ginela dla calej rodziny V-FLEXA/V-FLECTO.
  const marks = parseTechnicalMarks(record.bieznik, record.producent, record.rozmiar, record.nazwa);
  ...
  const result = {
    ...
    marka: emptyToNull(record.producent),
    // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): FarmPro/FarmPRO/FARMPRO -> FARM PRO,
    // usuwamy pelne frazy STEEL BELTED/HIGH SPEED z modelu (tokeny SB/HS dopisywane osobno
    // w nazwaKoncowa nizej z result.sb/result.hs, wiec nie zostaja w tekscie modelu).
    // POPRAWKA 2026-07-21 (anomalia 4: DUBLET SB/HS): gdy dostawca wpisuje skrot "SB"/"HS"
    // wprost w nazwie handlowej (np. "AGRIMAX PROHARVEST SB E"), a jednoczesnie flaga
    // marks.sb/marks.hs jest ustawiona, token dopisywany byl DRUGI raz w nazwaKoncowa
    // ("...PROHARVEST SB E ... SB"). Usuwamy wolnostojacy token SB/HS z modelu WYLACZNIE gdy
    // odpowiednia flaga jest aktywna (i tak zostanie dodany z powrotem) — bez ryzyka utraty
    // tokenu, ktory nie jest flaga.
    model: emptyToNull(record.bieznik)
      ? emptyToNull(record.bieznik)
          .replace(/\bfarm\s*pro\b/gi, 'FARM PRO')
          .replace(/\bsteel\s*belted\b/gi, '')
          .replace(/\bhigh\s*speed\b/gi, '')
          .replace(marks.sb ? /\bSB\b/gi : /(?!)/g, ' ')
          .replace(marks.hs ? /\bHS\b/gi : /(?!)/g, ' ')
          .replace(/\s+/g, ' ')
          .trim()
      : null,
    rozmiar: size.rozmiar,
  ...
  // POPRAWKA 2026-07-21 (kolejnosc VF/CFO/NRO/CHO, decyzja Anny): wczesniej VF/IF ORAZ
  // CFO/NRO/CHO byly doklejane na SAMYM KONCU nazwy (po TL). Wzorzec docelowy (jak
  // CEAT/Alliance): VF/IF sklejone PRZED rozmiarem (np. "VF380/90R50"), a CFO/NRO/CHO
  // ZARAZ PO rozmiarze (np. "VF600/60R28 NRO CEAT ..."). Odtwarzamy tu szyk buildTyreName()
  // ([VF]rozmiar -> NRO/CHO/CFO -> marka -> model -> LI/SI -> PR -> SB/HS -> TL), ale
  // ZACHOWUJEMY lokalny formatLiSi(load,speed) zamiast wewnetrznej logiki buildTyreName
  // (ktora gubi skrocony zapis "170A8/B" = 1 nosnosc + wiele predkosci, obslugiwany tylko
  // przez formatLiSi) - inaczej Agrorami tracilby czesc indeksu predkosci.
  const sizeForName = result.vfIf ? `${result.vfIf}${result.rozmiar}` : result.rozmiar;
  result.nazwaKoncowa = [
    sizeForName,
    result.nro ? 'NRO' : null,
...
function normalizeGri(record) {
  ...
  // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): dostawca GRI wpisuje czasem
  // "GRIPEX" bez spacji -> GRIP EX. Robimy to przed dalszym uzyciem bieznik/model.
  const bieznik = cleanText(record.model || record.bieznik || '').replace(/\bGRIPEX\b/gi, 'GRIP EX');
  const fullSpec = `${bieznik} ${spec}`.trim();
  const size = parseSize(spec);
  // POPRAWKA 2026-07-21: doklejamy bieznik do wykrywania SB/SF/HS (wczesniej tylko spec).
  const marks = parseTechnicalMarks(spec, bieznik);
...
function buildTyreName(parts) {
  ...
  const tech = [
    ...
    parts.sb ? 'SB' : null,
    parts.sf ? 'SF' : null,
    // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): HS (HIGH SPEED) analogicznie do SB.
    parts.hs ? 'HS' : null,
...
function normalizeBohnenkamp(row) {
  ...
  const marks = {
    ...
    // POPRAWKA 2026-07-21: SB/HS moga byc doklejone do samej nazwy (np. "... STEEL BELTED"),
    // nie tylko do spec - wczesniej sb/sf/hs (przez spread parseTechnicalMarks(spec)) gubily
    // przypadek gdy oznaczenie bylo tylko w nazwie.
    sb: parseTechnicalMarks(cleanName).sb || parseTechnicalMarks(spec).sb,
    sf: parseTechnicalMarks(cleanName).sf || parseTechnicalMarks(spec).sf,
    hs: parseTechnicalMarks(cleanName).hs || parseTechnicalMarks(spec).hs,
  ...
  // POPRAWKA 2026-07-21 (standaryzacja, decyzja Anny): FarmPro/FarmPRO/FARMPRO -> FARM PRO,
  // usuwamy pelne frazy STEEL BELTED/HIGH SPEED z modelu (tokeny SB/HS dopisywane osobno
  // przez buildTyreName z marks.sb/marks.hs powyzej, wiec nie zostaja w tekscie modelu).
  if (model) {
    model = model
      .replace(/\bfarm\s*pro\b/gi, 'FARM PRO')
      .replace(/\bsteel\s*belted\b/gi, '')
