...
//
// ⚠ ZAKRES: DWADZIEŚCIA Z DWUDZIESTU SIEDMIU TRAS MODUŁU — pięć z bloku 10a (fundament),
// pięć z 10b (ceny), sześć z 10c (EAN) i cztery z 10d (dostawcy); każdy blok ma niżej własną
// sekcję. Pozostałe siedem (dostępność, rotacja, cykl życia, export) dowożą bloki 10e i 10f.
// Dokładając je tutaj, trzymaj się układu tego pliku: jedna funkcja = jedna trasa, SQL
// przepisany dosłownie, limity i progi jako nazwane stałe. Pomocnik wspólny dla kilku bloków
// wydziel NAD ich sekcje, jak `czyJestHistoria`.
...
// ─── WSPÓLNE POMOCNIKI BLOKÓW 10b–10f ──────────────────────────────────────────────────
//
// Porty `num`, `round` i `hasHistory` z oryginału (`analytics_module.cjs:51-58`). Każdy z nich
// był niezależnie dopisywany przez dwa różne bloki równoległych sesji — stoją tu, NAD sekcjami
// bloków, żeby kolejna sesja zastała je gotowe zamiast dokładać trzecią kopię.
 ...
 *
 * ⚠ ZWRACA LICZBĘ, NIE NAPIS, i to jest istotne: `toFixed` dałby string, a fixtures niosą
 * w tych polach liczby (`stats.avg: 1561.39`). Bloki 10b (`stats` w `prices/product-history`)
 * i 10c (przeliczenia EAN) doszły do tego portu niezależnie — stoi tu, nad sekcjami obu,
 * żeby trzeci blok nie dopisał trzeciej kopii.
 */
