...
// ── Iteracja 13d-1 (ticket 45) — MODEL WARIANTOWY, migracja `007_selly_products_warianty.sql`.
//
...
// Cena i stan siedzą w Selly NA WARIANCIE, nie na produkcie — stąd cała przebudowa
// (backlog #60, CHANGELOG 2026-09-07 20:03).
//
// ⚠ `bridgeKod` NIE jest już unikatowy w DDL (stara tabela miała `UNIQUE`) — unikatowość
// przeniosła się na parę (kod_importu, dostawca). Join `GET /api/selly/status`
// (`repos/selly.ts`) po `bridge_kod = products.kod` działa dalej, bo `bridge_kod` niesie
// pełny kod Bridge (np. `MO2_19539`), czyli i tak jedną wartość na parę.
...
(table) => [
	unique("selly_products_kod_importu_dostawca_unique").on(table.kodImportu, table.dostawca),
...
// Stara tabela sprzed modelu wariantowego — zachowana przez `ALTER TABLE ... RENAME`
// (u Ani 2174 przestarzałe wpisy MO1/MO2). NIC JEJ NIE CZYTA; model istnieje po to, żeby
// migracja miała odpowiednik w kodzie i żeby ewentualny przyszły odczyt archiwum nie musiał
// iść surowym SQL-em.
