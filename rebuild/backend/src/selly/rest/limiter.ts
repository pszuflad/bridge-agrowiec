/**
 * Token bucket dla API Selly — port `origin/main:mirror/backend/selly/rate_limiter.cjs`
 * (karta I15.6, ticket 108; plik bez zmian od 07.09, więc treść portu z cofniętego ticketu 45
 * pozostaje wierna).
 *
...
/** Statystyki `getStats()` — w oryginale wystawia je `GET /api/selly/sync-status` (`routes_sync.cjs`, karta I15.8). */
 ...
 * (`rate_limiter.cjs:56`). Jeden proces = jeden klucz API Selly = jedno okno limitu,
