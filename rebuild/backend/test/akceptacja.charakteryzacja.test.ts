 ...
 *
 * ⭐ CHARAKTERYZACJA DOWODZI TEŻ DECYZJI D3 (pominięcie narzutów/promocji do I4). Oryginał
 * wykonuje pełną gałąź cenową (`__bridgePickMarkup`/`__bridgePickPromo` są wycięte NAPRAWDĘ,
 * nie jako zaślepki) na pustych tabelach `markups`/`promotions` — i wychodzi z tym samym
 * wynikiem, co nasz port, który tej gałęzi nie ma. Gdyby ktoś wpisał regułę, obie strony
 * natychmiast by się rozjechały.
 */
