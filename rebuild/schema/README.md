...
| `012_staging_polityka.sql` | **Karta I15.4a** (`124-FEATURE-fundament-stagingu`, backlog #99/#103/#104/#106): sześć tabel polityki stagingu — `staging_matches`, `supplier_feed_state`, `supplier_feed_versions`, `product_absence_checks`, `product_auto_suspensions`, `staging_absence_decisions` — i dwa indeksy unikalne (`staging_one_current_product` na `staging_items(dostawca,kod)`, częściowy `staging_absence_one_choice ON staging_absence_decisions(supplier,selected_source_code) WHERE selected_source_code IS NOT NULL`); DDL bajt w bajt z `git show 88fa31c:db/schema.sql:332-347`. Idempotentna przez natywne SQLite `CREATE … IF NOT EXISTS` — **bez dyrektywy runnera** (inaczej niż `ALTER TABLE` w 002/011 i przebudowa w 013). Przed indeksem unikalnym sprząta duplikaty `staging_items` (zostaje `MAX(id)` per `dostawca, kod`): na kopii `db/snapshot.db` usuwa 137 wierszy (2639→2502, po tym jak 006 wcześniej zabrała 723 z surowych 3362 — obie liczby prawdziwe, mierzą różne etapy). ⚠ **Na produkcji migracja jest w całości no-opem** — wszystkie sześć tabel i oba indeksy tam już istnieją (Ania założyła je 22–23.09) — więc, inaczej niż 002/003/013, **nie wymaga żadnego kroku ręcznego przy cutoverze**. |
| `013_selly_products_warianty.sql` | **Karta I15.6** (`108-FEATURE-selly-rest-discovery-delta`, backlog #60): `selly_products` → model wariantowy. Stara tabela przemianowana na `selly_products_old` (zachowuje dane i indeks `idx_selly_products_kod`), nowa z kluczem `(kod_importu, dostawca)`, `selly_variant_id`, `feature_id_magazyn` i sześcioma indeksami — DDL verbatim z `origin/main:db/schema.sql`. Danych nie przenosi (Ania też nie). ⚠ **Na bazie produkcji pada** (oba obiekty już istnieją od 07.09) i wycofuje się w całości — przy cutoverze weryfikacja kształtu + ręczny wpis do `_migracje`, jak dla 002. |
